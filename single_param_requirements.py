...
def run_simulation(param):
    ...
    opt_params['emission_fidelity'] = float(param)
...
if __name__ == '__main__':
    ...
    with Pool(processes=80) as pool:
        results = pool.map(run_simulation, emission_fidelity_values) #EDIT THIS LINE TO CHANGE THE PARAMETER

    # Print results to console
    for param, avg_outcome, avg_runtime, avg_attempts in results:
        print(f"emission_fidelity: {param}, successprob: {avg_outcome} +/- {confidence}, avg attempts: {avg_attempts},  avg simulated time: {convert_seconds(avg_runtime, 1e6)} for {num_runs} runs")
    ...
    with open(csv_file_path, 'w', newline='') as csvfile:
        fieldnames = ['emission_fidelity', 'avg_outcome', 'avg_runtime', 'avg_attempts']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        for param, avg_outcome, avg_runtime, avg_attempts in results:
            writer.writerow({'emission_fidelity': param, 'avg_outcome': avg_outcome, 'avg_runtime': avg_runtime, 'avg_attempts': avg_attempts})
